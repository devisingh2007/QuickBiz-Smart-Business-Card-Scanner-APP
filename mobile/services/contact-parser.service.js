const DESIGNATION_KEYWORDS = ['engineer', 'developer', 'manager', 'designer', 'director', 'president', 'founder', 'ceo', 'cfo', 'cto', 'coo', 'vp', 'architect', 'specialist', 'consultant', 'analyst', 'lead', 'partner', 'executive', 'associate', 'officer', 'representative', 'owner', 'proprietor', 'head', 'vice president', 'programmer', 'chief', 'principal', 'administrator', 'strategist', 'adviser', 'advisor', 'coordinator', 'leader', 'expert'];
const COMPANY_KEYWORDS = ['ltd', 'inc', 'corp', 'co.', 'corporation', 'solutions', 'technologies', 'systems', 'startup', 'group', 'enterprises', 'industries', 'services', 'labs', 'agency', 'tech', 'software', 'digital', 'global', 'llc', 'pvt', 'incorporated', 'ventures'];
const ADDRESS_KEYWORDS = ['street', 'road', 'st.', 'rd.', 'ave', 'avenue', 'highway', 'hwy', 'building', 'bldg', 'floor', 'fl', 'suite', 'ste', 'city', 'state', 'zip', 'pincode', 'post', 'box', 'phase', 'sector', 'zone', 'nagar', 'chowk', 'vihar', 'enclave', 'complex', 'tower', 'india', 'usa', 'landmark', 'opp', 'near', 'beside', 'behind', 'block', 'plot', 'district', 'cantt', 'industrial area', 'park', 'plaza', 'house', 'lane', 'villa', 'apartment'];
const COMMON_EMAIL_PROVIDERS = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'yahoo.co.in', 'icloud.com', 'mail.com'];

/**
 * Extracts structured contact details (name, email, phone, company, etc.) from OCR text lines.
 */
export function parseOcrResult(ocr) {
  // Fix minor OCR scanning spacing errors (e.g. "user @ domain.com")
  const rawLines = (ocr.lines || []).map(l => {
      ...
      for (const m of matches) {
        ...
        if (digitsOnly.length >= 7 && digitsOnly.length <= 15) {
          ...
          if (lower.includes('fax') || lower.includes('f:')) {
            type = 'fax';
            label = 'Fax';
          } else if (lower.includes('office') || lower.includes('work') || lower.includes('off') || lower.includes('tel') || lower.includes('ph') || lower.includes('land') || lower.includes('landline')) {
            type = 'office';
            label = 'Office';
          }
  ...
  // 6. Extract Name
  let nameIdx = -1;
  const nameCandidates = nonAddressLines.map((line, idx) => ({
    line,
    idx
  })).filter(({
    line,
