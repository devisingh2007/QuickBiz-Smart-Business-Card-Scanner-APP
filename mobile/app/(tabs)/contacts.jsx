...
export default function ContactsScreen() {
  ...
  useEffect(() => {
    ...
    const filtered = allContacts.filter(contact => {
      const matchesSearch = !query || contact.name.toLowerCase().includes(query) || contact.company && contact.company.toLowerCase().includes(query) || contact.designation && contact.designation.toLowerCase().includes(query) || (contact.emails || []).some(e => e.value.toLowerCase().includes(query)) || (contact.phones || []).some(p => p.value.toLowerCase().includes(query));
  ...
  return <SafeAreaView style={[styles.safeArea, {
    ...
    })} /> : <FlatList data={contacts} keyExtractor={(item, index) => item.id || index.toString()} contentContainerStyle={styles.listContent} renderItem={({
      item,
      index
    }) => <ContactRow contact={item} onPress={() => router.push({
